...
object MetaDataFactory {
  def apply(appConfig: String): Basemetadata = {
    val parsedJson = parse(appConfig)
    implicit val formats = DefaultFormats

    (parsedJson \ "process_type").extract[String].toLowerCase match {
      case "load" => {
      ...
      case "denorm" => {
        ...
        val jobType = (parsedJson \ "denorm_metadata" \ "job_type").extract[String]
        DenormMetaData(SparkJobType.withName(jobType), locationSource, locationDest, generatedSparkConf)
